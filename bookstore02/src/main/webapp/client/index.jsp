						...
						<c:forEach items="${products }" var="product">
							<td style="width:80; text-align:center">
								<a href="${pageContext.request.contextPath}/client/products/findProductById.do?id=${product.id}">
									<img src="${pageContext.request.contextPath }${product.imgurl}" width="102" height="130" border="0" /> 
								</a>
								<br /> 
								<a href="${pageContext.request.contextPath}/client/products/findProductById.do?id=${product.id}">${product.name}</a>
								<%-- <br />作者:${pArray[2] } --%>
							</td>
						</c:forEach>
